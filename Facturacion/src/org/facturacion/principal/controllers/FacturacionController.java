			...
			if(e.getSource()==principal.panel.comboBoxTipoFactura)
			{
				if(principal.panel.comboBoxTipoFactura.getSelectedIndex()>-1)
				{
					...
					try {
						...
						principal.panel.lblNumeroFacturaValor.setText(serviceFactura.getNumeracion(tipoFactura.getId()).toString());
					} catch (Exception e1) {
						// TODO Auto-generated catch block
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource()==principal.panel.btnCambiarCantidad)
			{
				...
				if(row>-1)
				{
					...
					{
						Long codigo=Long.parseLong((String)principal.panel.table.getValueAt(row, 1).toString());
						String input = JOptionPane.showInputDialog(null, "Ingresa una nueva cantidad:", "Solicitud de valor", JOptionPane.QUESTION_MESSAGE);
						if(input!=null)
						{
			...
			if(e.getSource()==principal.panel.btnCambiarPrecio)
			{
				...
				if(row>-1)
				{
					...
					{
						Long codigo=Long.parseLong((String)principal.panel.table.getValueAt(row, 1).toString());
						String input = JOptionPane.showInputDialog(null, "Ingresa un nuevo precio:", "Solicitud de valor", JOptionPane.QUESTION_MESSAGE);
						if(input!=null)
						{
			...
			{
				String input = JOptionPane.showInputDialog(null, "Ingresa el porcentaje de descuento", "Solicitud de valor", JOptionPane.QUESTION_MESSAGE);
				if(input!=null)
				{
					try
					{
